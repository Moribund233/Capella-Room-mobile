...
function appendMessageToCache(roomId: string, msg: Message) {
...
function updateMessageInCache(roomId: string, messageId: string, updates: Partial<Message>) {
  const keys = messageKeys.list(roomId);
  const data = queryClient.getQueryData<InfiniteData<MessagesResponse>>(keys);
  if (!data) return;
  ...
  useEffect(() => {
    ...
    unsubscribers.push(
      ws.on("MessageEdited", (payload: any) => {
        updateMessageInCache(payload.message_id, payload.room_id, {
